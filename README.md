...
为准。页面不提供独立 USB 按钮；静默扫描为空时，同一个“搜索设备”动作会直接调用
严格匹配 OVIS 配置接口的 `requestDevice()`，首次授权取消不会中断网络扫描。
...
返回空数组；因此首次为空时会在搜索动作中显示 Chrome 原生选择器。每次只能授权一台，
本会话内再次搜索可继续授权下一台。企业策略存在时会直接返回全部匹配设备，不弹窗。
