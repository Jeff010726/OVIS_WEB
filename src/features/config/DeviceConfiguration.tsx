  ...
  return (
    <div className="configuration-page">
      <header className="connected-device-bar">
        ...
        <dl className="connected-device-bar__meta">
          <div><dt>设备地址</dt><dd>{endpointLabel(selectedDevice.apiBaseUrl)}</dd></div>
          <div><dt>固件 / Manager</dt><dd>{device.firmware_version} / {device.manager_version}</dd></div>
      ...
      {configuration.status === "error" && (
        <div className="configuration-loading" role="alert">
          ...
          <button className="button button--secondary" type="button" onClick={() => void configuration.load()}>
            <RefreshCw size={15} />重试
          </button>
        </div>
      )}

      {configuration.capabilities && configuration.draft && configuration.status !== "error" && configuration.status !== "loading" && (
        <main className="configuration-workspace">
          <div className="configuration-toolbar">
            ...
            <div className="configuration-toolbar__actions">
              ...
              >
                {isBusy ? <LoaderCircle className="button-spinner" size={16} /> : <Save size={16} />}
                保存并应用
              </button>
            </div>
          </div>
          ...
          {isBusy && (
            <div className="operation-progress" role="status">
              ...
              <span className="operation-progress__track">
                <span style={{ width: `${configuration.task ? taskProgress : 12}%` }} />
              </span>
            </div>
          )}

          {configuration.outcome && (
            <div className={`config-notice config-notice--${configuration.outcome.type}`} role="status">
              {configuration.outcome.type === "success" ? <CheckCircle2 size={17} /> : <AlertTriangle size={17} />}
              <div>
                <strong>{configuration.outcome.message}</strong>
                {configuration.outcome.type === "error" && (
                  <span>{configuration.outcome.rolledBack ? "自动回滚成功，页面已重新读取设备配置。" : "设备未确认自动回滚，请检查当前配置。"}</span>
                )}
              </div>
              <button className="icon-button" type="button" aria-label="关闭提示" onClick={configuration.dismissOutcome}>
                <X size={14} />
              </button>
            </div>
          )}
          ...
          {configuration.validation && (!configuration.validation.valid || configuration.validation.warnings.length > 0) && (
            <div className={`config-notice ${configuration.validation.valid ? "config-notice--warning" : "config-notice--error"}`} role="alert">
              <AlertTriangle size={17} />
              <div>
                <strong>{configuration.validation.valid ? "配置包含警告" : "配置校验未通过"}</strong>
                {[...configuration.validation.errors, ...configuration.validation.warnings].map((issue) => (
                  <span key={`${issue.field}-${issue.code}`}>{issue.message}</span>
                ))}
              </div>
            </div>
          )}
          ...
          <fieldset className="configuration-form" disabled={isBusy}>
            <section className="config-section" aria-labelledby="video-config-heading">
              <div className="config-section__heading">
                <Video size={18} />
                <div><span>01</span><h3 id="video-config-heading">视频码流</h3></div>
              </div>
            ...
            <section className="config-section" aria-labelledby="feature-config-heading">
              <div className="config-section__heading">
                <Settings2 size={18} />
                <div><span>02</span><h3 id="feature-config-heading">画面与智能检测</h3></div>
              </div>
              <div className="feature-list">
                <div className="feature-row feature-row--simple">
                  <div className="feature-row__identity">
                    <span aria-hidden="true"><Video size={17} /></span>
                    <div>
                      <strong>OSD 叠加</strong>
                      <small>{configuration.capabilities.features.osd ? (configuration.draft.overlay.enabled ? "已启用" : "已关闭") : "设备不支持"}</small>
                    </div>
                  </div>
                  <Toggle
                    ...
                    label="启用 OSD"
                    onChange={(checked) => configuration.updateDraft((draft) => { draft.overlay.enabled = checked; })}
                  />
                </div>
                {detectionRows.map(({ key, ...row }) => (
                  <DetectionRow
                    ...
                    onToggle={(checked) => updateDetection(key, "enabled", checked)}
                    onValue={(value) => updateDetection(key, "value", value)}
                  />
                ))}
              </div>
            </section>
